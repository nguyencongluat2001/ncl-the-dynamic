...
JS_ArticlesBook.prototype.update = function(oForm) {
    if ($(oForm).valid()) {
        ...
        var FK_CATEGORY = '';
        if (value == '') {
            NclLib .alertMessage('danger', "Nội dung không được để trống");
            return false;
        }
...
JS_ArticlesBook.prototype.submit_document = function(oForm) {
    ...
    if (i == 0) {
        NclLib .alertMessage('danger', "Bạn chưa chọn văn bản cần thêm");
        return false;
    }
    if (i > 1) {
        NclLib .alertMessage('danger', "Bạn chỉ được chọn một văn bản để thêm");
        return false;
    }
